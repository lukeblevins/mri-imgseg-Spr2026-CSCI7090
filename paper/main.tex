...
\item \textbf{RQ5:} How can seizure prediction be improved through multimodal modeling that integrates EEG time-series features and clinical metadata while overcoming weak cross-patient generalization and inconsistent benchmarking practices?
...
\nocite{*}
\section{Datasets}
\noindent This study will utilize both real-time as well as static datasets to evaluate performance of models for both data forms. Static datasets are more commonly used in seizure prediction as these datasets provide standarized and cleaned data for training and evaluation. However, real-time data better simulates data that would be passed into a model during real time environments. Real-Time data will be collected BrainFlow which obtains EEG data from connected devices through an API. In addition, two widely used open-source datasets will be used, the CHB-MIT Scalp EEG dataset as well as the Siena Scalp EEG Database. These datasets provide a diverse collection of EEG data for models to train and make predictions on more general data.
\section{Data Acquisition and Description}
\noindent The CHB-MIT and Siena Scalp EEG datasets are highly relevant to epileptic seizure prediction using EEG time-series data. Both datasets are collected via scalp electrodes, making them non-invasive. Datasets collected through non-invasive means are far more common and available for use in machine learning when compared to more invasive methods, such as intracranial EEG data. Additionally, the CHB-MIT dataset consists of pediatric subjects, whereas the Siena dataset consists of adult subjects. The diversity in age and seizure characteristics across the datasets will help improve the generalization of our model to new patients. Both datasets are also labeled, making it perfect for our team's supervised machine learning approach. Additionally, since the datasets consist of raw seizure data through EDF files, we are unable to create the dataset description and dictionary. Transitioning to the exploitation of real time data, this data is streamed via the brainflow API. This is an experimental part of the project as the data is unlabeled, making integration with static labeled data difficult. Each row of the dataset contains a 30 second eeg window with a binary label determining whether the window is an interictal or preictal phase based on timestamps. This form of data is addressing a key gap discussed among literature reviews as real time data is the standard data which would be found in a real-time environment in which many studies do not utilize. Missing values are handled accordingly with missing channels and bad windows being dropped along with small time gaps being interpolated. EEG features are converted to floats, time data is converted to datetime or numeric, and labels are handled as binary. Z score and IQR methods are utilized for outlier detection such as high amplitudes and flatlines. Feature engineering is utilized through examples such as Hjorth parameters and line length which distinguish amplitude and frequency changes along with detecting abrupt changes in a signal, respectively. Scaling is used to handle wide varieties in magnitude. 
\printbibliography
